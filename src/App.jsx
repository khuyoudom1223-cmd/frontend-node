...
export default function App() {
  ...
  // Handle Social Login (OAuth Callback Redirect) on mount
  useEffect(() => {
    const params = new URLSearchParams(window.location.search);
    const token = params.get('token');
    if (token) {
      // Clear URL parameter so it doesn't stay in the history
      ...
      const fetchOAuthUser = async () => {
        try {
          ...
          if (response.ok && payload?.success) {
            ...
            triggerNotification('Successfully logged in with social account!', 'success');
            setCurrentTab('home');
          } else {
            triggerNotification('Failed to fetch user profile after social auth', 'error');
          }
        } catch (err) {
          triggerNotification('Could not connect to authentication server', 'error');
        }
      };
            ...
            <div className="p-8 space-y-6">
              ...
              <div className="space-y-3 mt-4">
                <div className="relative flex py-1 items-center">
                  ...
                  <span className="flex-shrink mx-3 text-[10px] font-bold text-slate-400 uppercase tracking-wider">Or continue with</span>
                  <div className="flex-grow border-t border-slate-100"></div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <button
                    onClick={() => {
                      triggerNotification('Redirecting to Google Auth...', 'info');
                      window.location.href = `${API_BASE_URL}/api/auth/google`;
                    }}
                    type="button"
                    className="flex items-center justify-center gap-2 px-3 py-2.5 border border-slate-200 rounded-xl bg-white text-xs font-bold text-slate-700 hover:bg-slate-50 hover:border-slate-300 transition duration-200 cursor-pointer shadow-sm"
                  >
                  ...
                  <button
                    onClick={() => {
                      triggerNotification('Redirecting to Facebook Auth...', 'info');
